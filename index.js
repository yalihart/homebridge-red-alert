...
class RedAlertPlugin {
  constructor(log, config, api) {
    ...
    this.chromecastVolume = Number.isFinite(config.chromecastVolume) ? config.chromecastVolume : 30;
    this.useChromecast = config.useChromecast !== false;
    this.chromecastTimeout = Number.isFinite(config.chromecastTimeout) ? config.chromecastTimeout : 30;
    this.wsUrl = config.wsUrl || 'ws://ws.cumta.morhaviv.com:25565/ws';
    this.reconnectInterval = Number.isFinite(config.reconnectInterval) ? config.reconnectInterval : 5000;
    this.serverPort = Number.isFinite(config.serverPort) ? config.serverPort : 8095;
  ...
  validateConfig() {
    try {
      ...
      if (!Array.isArray(this.selectedCities) || this.selectedCities.length === 0) {
        this.log.warn('No valid cities specified in config, monitoring all alerts');
        this.selectedCities = [];
      }
      if (this.useChromecast && Array.isArray(this.config.chromecastVolumes)) {
        this.config.chromecastVolumes.forEach((v, i) => {
          if (!v.deviceName || !Number.isFinite(v.volume)) {
            this.log.warn(`Invalid Chromecast volume config at index ${i}, ignoring`);
          }
        });
      }
    } catch (error) {
  ...
  initializeChromecastClient() {
    try {
      ...
      this.chromecastClient.on('device', (device) => {
        try {
          if (!device || !device.host || !device.friendlyName) {
            this.log.warn(`Invalid Chromecast device data: ${JSON.stringify(device)}`);
            return;
          }
          if (typeof device.play !== 'function' || typeof device.setVolume !== 'function') {
            this.log.warn(`Chromecast '${device.friendlyName}' lacks playback functions`);
            return;
          }
          ...
          if (this.config.chromecastVolumes && !this.devices.some(d => 
            this.config.chromecastVolumes.some(v => v.deviceName === d.friendlyName))) {
            this.log.warn('Some configured Chromecast devices not discovered');
          }
        } catch (error) {
          this.log.error(`Error processing Chromecast device: ${error.message}`);
        }
      });
      ...
      this.devices = [];
      this.log.warn('Chromecast functionality disabled due to initialization failure');
    }
  }
  ...
  setupWebSocket() {
    try {
      ...
      this.wsClient.on('open', () => {
        ...
        this.log.info(`Monitoring cities: ${this.selectedCities.join(', ') || 'all'}`);
  ...
  handleAlertMessage(message) {
    try {
      ...
      const relevantAreas = areas.filter(area => this.selectedCities.length === 0 || this.selectedCities.includes(area));
  ...
  playChromecastMedia(isTest) {
    try {
      ...
      this.log.info(`Playing ${isTest ? 'test' : 'alert'} on ${this.devices.length} devices`);
      const mediaUrl = isTest ? `${this.baseUrl}/test-video` : `${this.baseUrl}/alert-video`;

      this.devices.forEach((device) => {
        if (!device || typeof device.play !== 'function') {
          this.log.warn(`Invalid device: ${device?.friendlyName || 'unknown'}`);
          return;
        }
  ...
  playWithRetry(device, mediaUrl, retries) {
    try {
      device.play(mediaUrl, (err) => {
        if (err && retries > 0) {
          this.log.warn(`Retrying playback on ${device.friendlyName} (${retries} left)`);
          setTimeout(() => this.playWithRetry(device, mediaUrl, retries - 1), 2000);
        } else if (err) {
          this.log.error(`Failed to play on ${device.friendlyName}: ${err.message}`);
        } else {
          ...
          device.setVolume(deviceVolume / 100, (err) => {
            if (err) {
              this.log.warn(`Failed to set volume on ${device.friendlyName}: ${err.message}`);
            } else {
              this.log.debug(`Volume set to ${deviceVolume}% on ${device.friendlyName}`);
            }
          });
        }
      });
    } catch (error) {
      this.log.error(`Error in playWithRetry on ${device.friendlyName}: ${error.message}`);
    }
  }
